...
contract ReserveTest is DSTest {
    ...
    function assertPluginDeposited(TestReservePlugin plugin, IERC20 forToken,
        uint256 expected) public {
            assertEq(plugin.deposited(forToken), expected, "Invalid plugin deposited");
        }

    function assertPluginBalance(TestReservePlugin plugin, IERC20 forToken,
        uint256 expected) public {
    ...
    function testTokensDontMixBetweenPlugins() public {
        // set noPlugin - plugin1
        // deposit 3 - 5
        // check deposited check plugindeposited check balance for each
        // withdraw 1 from each
        // check deposited check plugindeposited check balance for each
    }
}
