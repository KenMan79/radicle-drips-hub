...
library Drips {
    ...
    function _defaultEnd(
        ...
        DefaultEndReceiver[] memory defaults = new DefaultEndReceiver[](receivers.length);
        uint256 length = 0;

        for (uint256 i = 0; i < receivers.length; i++) {
            DripsReceiver memory receiver = receivers[i];
            require(receiver.amtPerSec != 0, "Drips receiver amtPerSec is zero");
            require(i == 0 || _isOrdered(receivers[i - 1], receiver), "Receivers not sorted");
            uint64 start = receiver.start;
            if (start < dripsStart) start = dripsStart;
            if (receiver.duration == 0) {
                length = _addDefaultEndReceiver(defaults, length, start, receiver.amtPerSec);
            } else {
    ...
    struct DefaultEndReceiver {
        uint64 start;
        uint128 amtPerSec;
    }

    /// @notice Adds a `DefaultEndReceiver` to the list of receivers while keeping it sorted.
    /// @param receivers The list of receivers, must be sorted by start
    ...
    function _addDefaultEndReceiver(
        DefaultEndReceiver[] memory receivers,
        uint256 length,
        uint64 start,
        uint128 amtPerSec
    ) private pure returns (uint256 newLength) {
        for (uint256 i = 0; i < length; i++) {
            DefaultEndReceiver memory receiver = receivers[i];
            if (receiver.start == start) {
                receiver.amtPerSec += amtPerSec;
                return length;
            }
            if (receiver.start > start) {
                ...
                receiver = DefaultEndReceiver(start, amtPerSec);
                return length + 1;
            }
        }
        receivers[length] = DefaultEndReceiver(start, amtPerSec);
        return length + 1;
    }
    ...
    function _receiversDefaultEnd(
        ...
        for (uint256 i = 0; i < length; i++) {
            uint64 start = receivers[i].start;
            if (start >= end) break;
    ...
    /// @param startBalance The balance when drips have started
    ...
    function _updateReceiverStates(
        ...
        while (true) {
            ...
            if (pickCurr && pickNew) {
                ...
                _clearDeltaRange(deltas, cycleSecs, currStart, newStart, currRecv.amtPerSec);
                _setDeltaRange(deltas, cycleSecs, currEnd, newEnd, currRecv.amtPerSec);
            } else if (pickCurr) {
